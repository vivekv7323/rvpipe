...
def singleFileRV(path, cSplines, minima, maxima, boxList):
        ...
        class LineAnalysis(object):
            def __init__(self, params):
                self.params = params
            def __call__(self, j):
                fluxCont, errorCont, i = self.params
                box = boxList[i][j]
                ...
                if ((len(indices[0]) > 10) & (len(indices[0]) < 100) & (~np.isnan(fluxSpec).any())):
                        ...
                        lineWidthOrd = np.abs(waveLine[waveLine < lineMin][np.argmin(np.abs(fluxSpec[waveLine < lineMin] - halfMax))] -\
...
##                                bestpars = parsCov @ designMatrix @ covInv @ fluxSpec[location]
                                corrCoeffOrd = np.sqrt(np.square(parsCov[1][0])/(parsCov[1][1]*parsCov[0][0]))
                                RVOrd = (299792458*bestpars[1]/(bestpars[0]*lineMin))
                                RVErrorOrd = (299792458/lineMin)*np.sqrt((parsCov[1][1]/bestpars[0]**2)\
                ...
                return RVOrd, RVErrorOrd, corrCoeffOrd, lineWidthOrd
        ...
        for i in range(len(minima)):
            ...
            try:
                pool = Pool()
                output = np.asarray(pool.map(LineAnalysis((fluxCont, errorCont, i)), range(0, len(minima[i]))))
            finally:
                pool.close()
                pool.join()
                        
            RV.append(output[:,0])
            RVError.append(output[:,1])
            corrCoeff.append(output[:,2])
            lineWidth.append(output[:,3])
